...
class JsonLoader:
    ...
    def load_json_data(filename):
        with open('M2_LESSON_2_Working_with_Input/' + filename, encoding='UTF-8') as json_file:
...
class MainWindow(QWidget):
    def __init__(self, lesson_number=3, *args, **kwargs):
        ...
        self.progress_bar = ProgressBar(
            JsonLoader.load_json_data(os.path.join("..", "Page_order", "page_order_M2.json")), 1)
        self.init_ui()
    ...
    def save_log(self, log_type="time"):
        ...
        if not os.path.exists('M2_LESSON_2_Working_with_Input'):
            os.makedirs('M2_LESSON_2_Working_with_Input')

        # Guardar el archivo en la carpeta especificada
        filepath = os.path.join('M2_LESSON_2_Working_with_Input', filename)
