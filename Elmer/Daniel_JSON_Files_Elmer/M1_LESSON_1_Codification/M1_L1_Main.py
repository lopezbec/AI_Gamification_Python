...
class JsonWindow(QWidget):
    ...
    def openCommandLineUI(self, text):
        ...
        if not hasattr(self, 'commandLineWidget'):
            ...
            self.hideButton.setStyleSheet("background-color: orange; font-size: {self.styles['font_size_normal']}px; color: white;")
...
class MainWindow(QWidget):
    ...
    def save_log(self, log_type="time"):
        ...
        if log_type == "time":
            filename = f"{user}_Respuestas_M1_L1.csv"
        else:
            filename = f"{user}_Times_M1_L1.csv"
