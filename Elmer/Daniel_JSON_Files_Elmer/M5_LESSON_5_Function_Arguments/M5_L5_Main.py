...
from PyQt6.QtWidgets import QApplication, QWidget, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QStackedWidget, QRadioButton, QButtonGroup, QSizePolicy, QCheckBox, QTextEdit


class JsonLoader:
    @staticmethod
    def load_json_data(filename):
        with open('M5_LESSON_5_Function_Arguments/' + filename, encoding='UTF-8') as json_file:
...
class MainWindow(QWidget):
    def __init__(self, lesson_number=3, *args, **kwargs):
        ...
        self.setWindowTitle("Aprendiendo Python - Módulo 5, Lección 5")

        self.progress_bar = ProgressBar(JsonLoader.load_json_data(os.path.join("..", "Page_order", "page_order_M5.json")), 4)
        self.init_ui()
    ...
    def save_log(self, log_type="time"):
        ...
        filename = "M5_l4_Time.csv" if log_type == "time" else "M5_l4_Entradas_Salidas_Clics.csv"
        ...
        if not os.path.exists('M5_LESSON_5_Function_Arguments'):
            os.makedirs('M5_LESSON_5_Function_Arguments')

        # Guardar el archivo en la carpeta especificada
        filepath = os.path.join('M5_LESSON_5_Function_Arguments', filename)
    ...
    def switch_page(self, forward=True):
        ...
        # Sí se alcanza el final del recorrido de páginas, guardar el registro y cerrar la aplicación
            ...
            self.actualizar_progreso_usuario('Modulo5', 'Leccion5')
            self.close()
