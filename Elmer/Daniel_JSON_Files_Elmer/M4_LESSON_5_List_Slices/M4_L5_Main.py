...
class JsonLoader:
    ...
    def load_json_data(filename):
        with open('M4_LESSON_5_List_Slices/' + filename, encoding='UTF-8') as json_file:
...
class MainWindow(QWidget):
    def __init__(self, lesson_number=3, *args, **kwargs):
        ...
        self.progress_bar = ProgressBar(JsonLoader.load_json_data(os.path.join("..", "Page_order", "page_order_M4.json")), 4)
        self.init_ui()
    ...
    def save_log(self, log_type="time"):
        ...
        filename = "M4_L5_Time.csv" if log_type == "time" else "M4_L5_Entradas_Salidas_Clics.csv"
        ...
        if not os.path.exists('M4_LESSON_5_List_Slices'):
            os.makedirs('M4_LESSON_5_List_Slices')

        # Guardar el archivo en la carpeta especificada
        filepath = os.path.join('M4_LESSON_5_List_Slices', filename)
    ...
    def switch_page(self, forward=True):
        ...
        # Sí se alcanza el final del recorrido de páginas, guardar el registro y cerrar la aplicación
            ...
            self.actualizar_progreso_usuario('Modulo4', 'Leccion5')
            self.actualizar_leccion_completada('Modulo4', 'Leccion5')
            self.close()
