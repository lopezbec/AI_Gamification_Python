...
class JsonLoader:
    ...
    def load_json_data(filename):
        with open('M4_LESSON_2_List_Operations/' + filename, encoding='UTF-8') as json_file:
...
class MainWindow(QWidget):
    def __init__(self, lesson_number=3, *args, **kwargs):
        ...
        self.setWindowTitle("Aprendiendo Python - Módulo 4, Lección 2")

        self.progress_bar = ProgressBar(JsonLoader.load_json_data(os.path.join("..", "Page_order", "page_order_M4.json")), 1)
        self.init_ui()
    ...
    def save_log(self, log_type="time"):
        ...
        filename = "M4_l2_Time.csv" if log_type == "time" else "M4_L2_Entradas_Salidas_Clics.csv"
        ...
        if not os.path.exists('M4_LESSON_2_List_Operations'):
            os.makedirs('M4_LESSON_2_List_Operations')

        # Guardar el archivo en la carpeta especificada
        filepath = os.path.join('M4_LESSON_2_List_Operations', filename)
