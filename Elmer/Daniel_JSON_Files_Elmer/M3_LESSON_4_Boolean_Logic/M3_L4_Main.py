...
from PyQt6.QtWidgets import QApplication, QTextEdit, QWidget, QFrame, QVBoxLayout, QHBoxLayout, QLabel, QPushButton, QStackedWidget, QRadioButton, QButtonGroup, QSizePolicy, QCheckBox
...
class JsonLoader:
    ...
    def load_json_data(filename):
        with open('M3_LESSON_4_Boolean_Logic/' + filename, encoding='UTF-8') as json_file:
...
class MainWindow(QWidget):
    def __init__(self, lesson_number=3, *args, **kwargs):
        ...
        self.progress_bar = ProgressBar(JsonLoader.load_json_data(os.path.join("..", "Page_order", "page_order_M3.json")), 3)
        self.init_ui()
    ...
    def save_log(self, log_type="time"):
        ...
        filename = "M3_L4_Time.csv" if log_type == "time" else "M3_L4_Entradas_Salidas_Clics.csv"
        ...
        if not os.path.exists('M3_LESSON_4_Boolean_Logic'):
            os.makedirs('M3_LESSON_4_Boolean_Logic')

        # Guardar el archivo en la carpeta especificada
        filepath = os.path.join('M3_LESSON_4_Boolean_Logic', filename)
    ...
    def load_page_order(self):
        ...
        file_path = os.path.join('Page_order', 'page_order_M3.json')
    ...
    def switch_page(self, forward=True):
        ...
        # Sí se alcanza el final del recorrido de páginas, guardar el registro y cerrar la aplicación
            ...
            self.actualizar_progreso_usuario('Modulo3', 'Leccion4')
            self.actualizar_leccion_completada('Modulo3', 'Leccion4')
            self.close()
