...
class JsonLoader:
    ...
    def load_json_data(filename):
        with open('M1_LESSON_4_Mixing_things_up/' + filename, encoding='UTF-8') as json_file:
...
class MainWindow(QWidget):
    def __init__(self, lesson_number=3, *args, **kwargs):
        ...
        self.setWindowTitle("Aprendiendo Python - Módulo 1, Lección 4")

        self.progress_bar = ProgressBar(JsonLoader.load_json_data(os.path.join("..", "Page_order", "page_order_M1.json")), 3)
        self.init_ui()
    ...
    def save_log(self, log_type="time"):
        ...
        filename = "M1_L4_Time.csv" if log_type == "time" else "M1_L4_Entradas_Salidas_Clics.csv"
        ...
        if not os.path.exists('M1_LESSON_4_Mixing_things_up'):
            os.makedirs('M1_LESSON_4_Mixing_things_up')

        # Guardar el archivo en la carpeta especificada
        filepath = os.path.join('M1_LESSON_4_Mixing_things_up', filename)
    ...
    def switch_page(self, forward=True):
        ...
        # Sí se alcanza el final del recorrido de páginas, guardar el registro y cerrar la aplicación
            ...
            self.actualizar_progreso_usuario('Modulo1', 'Leccion4')
