...
class JsonLoader:
    ...
    def load_json_data(filename):
        with open('M5_LESSON_6_Returning_From_Functions/' + filename, encoding='UTF-8') as json_file:
...
class MainWindow(QWidget):
    def __init__(self, lesson_number=3, *args, **kwargs):
        ...
        self.setWindowTitle("Aprendiendo Python - Módulo 5, Lección 6")

        self.progress_bar = ProgressBar(JsonLoader.load_json_data(os.path.join("..", "Page_order", "page_order_M5.json")), 5)
        self.init_ui()
    ...
    def save_log(self, log_type="time"):
        ...
        filename = "M5_L6_Time.csv" if log_type == "time" else "M5_L6_Entradas_Salidas_Clics.csv"
        ...
        if not os.path.exists('M5_LESSON_6_Returning_From_Functions'):
            os.makedirs('M5_LESSON_6_Returning_From_Functions')

        # Guardar el archivo en la carpeta especificada
        filepath = os.path.join('M5_LESSON_6_Returning_From_Functions', filename)
    ...
    def switch_page(self, forward=True):
        ...
        # Sí se alcanza el final del recorrido de páginas, guardar el registro y cerrar la aplicación
            ...
            self.actualizar_progreso_usuario('Modulo5', 'Leccion6')
            self.close()
