...
class JsonLoader:
    ...
    def load_json_data(filename):
        with open('M5_LESSON_3_String_Functions/' + filename, encoding='UTF-8') as json_file:
...
class MainWindow(QWidget):
    ...
    def save_log(self, log_type="time"):
        ...
        filename = "M5_L3_Time.csv" if log_type == "time" else "M5_L3_Entradas_Salidas_Clics.csv"
        ...
        if not os.path.exists('M5_LESSON_3_String_Functions'):
            os.makedirs('M5_LESSON_3_String_Functions')

        # Guardar el archivo en la carpeta especificada
        filepath = os.path.join('M5_LESSON_3_String_Functions', filename)
