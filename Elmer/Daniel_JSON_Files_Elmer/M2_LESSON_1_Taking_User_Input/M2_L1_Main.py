...
class JsonWindow(QWidget):
    ...
    def handle_answer_click(self, answer_text):
        ...
        updated_hint = self.hint_label.text()[:self.blank_space_index] + answer_text + self.hint_label.text()[
...
class MainWindow(QWidget):
    def __init__(self, lesson_number, *args, **kwargs):
        ...
        self.setWindowTitle("Capturar entradas del usuario")
            ...
            if "draganddropSecuence" in data_block and data_block["draganddropSecuence"]:
                ...
                for label in drop_labels:
                    ...
                    if "multipleResponseVariant" in data_block and data_block["multipleResponseVariant"]:
                        ...
                        assignment_match = re.search(assignment_pattern, full_dropped_text)
                        print_match = re.search(print_pattern, full_dropped_text)

                        if assignment_match:
                            dropped_text = f"'{assignment_match.group(1)}'"  # Agregamos las comillas aquí
                        elif print_match:
                            dropped_text = print_match.group(1)
                        else:
                        ...
                        if correct_answer:
                            if correct_answer["text"] in full_dropped_text:
                                correct_count += 1
                                self.log_event(f"Correct Answer Selected: {full_dropped_text}", event_type="mouse")
                            else:
                                self.log_event(f"Incorrect Answer Selected: {full_dropped_text}", event_type="mouse")
    ...
    def switch_page(self, forward=True):
        ...
        # Sí se alcanza el final del recorrido de páginas, guardar el registro y cerrar la aplicación
            ...
            self.save_log(modulo=2, leccion=1)
            ...
            self.actualizar_progreso_usuario('Modulo2', 'Leccion1')
            self.actualizar_leccion_completada('Modulo2', 'Leccion1')
            update_lesson_status(self.usuario_actual, 'Modulo2', 'Leccion1', self.all_correct)
            ...
            update_lesson_dates(self.usuario_actual, "Modulo2", "Leccion_completada1")
            if are_lessons_completed_same_day(self.usuario_actual, "Modulo2") and not is_badge_earned(self.usuario_actual, 'modulo_rapido'):
                display_badge('modulo_rapido')
                update_badge_progress(self.usuario_actual, 'modulo_rapido')
            if are_two_lessons_completed_same_day(self.usuario_actual, "Modulo2") and not is_badge_earned(self.usuario_actual, 'doble_aprendizaje'):
