...
class MainWindow(QWidget):
    def __init__(self, lesson_number=3, *args, **kwargs):
        ...
        self.setWindowTitle("Operadores In-Place")
    ...
    def load_page_order(self):
        ...
        file_path =  os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'Page_order', 'page_order_M2.json')
    ...
    def actualizar_progreso_usuario(self, modulo, leccion_completada):
        try:
            print(f"Intentando abrir el archivo 'progreso.json' para el usuario {self.usuario_actual}.")
            with open(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'progreso.json'), 'r',
                      encoding='UTF-8') as file:
                progreso = json.load(file)

            print("Archivo 'progreso.json' cargado exitosamente.")
            progreso_usuario = progreso.get(self.usuario_actual, {})
            print(f"Progreso del usuario actual: {progreso_usuario}")

            # Verifica si el módulo existe para el usuario
            if modulo not in progreso_usuario:
                raise KeyError(f'El módulo {modulo} no existe para el usuario {self.usuario_actual}.')

            print(f"El módulo {modulo} existe para el usuario.")
            ...
            print(f"Lección completada: {leccion_completada}, desbloqueando la siguiente lección: {siguiente_leccion}.")

            # Desbloquea la siguiente lección en el mismo módulo si existe
            if siguiente_leccion in progreso_usuario[modulo]:
                progreso_usuario[modulo][siguiente_leccion] = True
                print(f"Siguiente lección {siguiente_leccion} desbloqueada en el módulo {modulo}.")
            else:
                # Si todas las lecciones están completadas, desbloquea la primera lección del siguiente módulo
                lecciones_modulo = progreso_usuario[modulo]
                todas_completadas = all(estado for estado in lecciones_modulo.values())
                print(f"Todas las lecciones del módulo {modulo} completadas: {todas_completadas}.")

                if todas_completadas:
                    numero_modulo_actual = int(modulo.replace("modulo_", ""))
                    siguiente_modulo = f'modulo_{numero_modulo_actual + 1}'
                    print(f"Desbloqueando el siguiente módulo: {siguiente_modulo}.")

                    # Verifica si el siguiente módulo existe y desbloquea la primera lección
                    if siguiente_modulo in progreso_usuario:
                        progreso_usuario[siguiente_modulo]["Leccion1"] = True
                        print(f"Primera lección desbloqueada en el módulo {siguiente_modulo}.")
            ...
            with open(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), 'progreso.json'), 'w',
                ...
                print(f"Progreso actualizado y guardado exitosamente en 'progreso.json'.")
    ...
    def actualizar_leccion_completada(self, id_modulo, leccion_completada):
        try:
            print(f"Intentando abrir el archivo 'leccion_completada.json' para el usuario {self.usuario_actual}.")
            with open(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                                   'leccion_completada.json'), 'r', encoding='UTF-8') as file:
                leccion_completada_data = json.load(file)

            print("Archivo 'leccion_completada.json' cargado exitosamente.")
            leccion_completada_usuario = leccion_completada_data.get(self.usuario_actual, {})
            print(
                f"Progreso de lecciones completadas para el usuario {self.usuario_actual}: {leccion_completada_usuario}")

            if id_modulo not in leccion_completada_usuario:
                leccion_completada_usuario[id_modulo] = {}
                print(f"Creando nuevo módulo {id_modulo} para el usuario {self.usuario_actual}.")

            leccion_completada_usuario[id_modulo][
                f"Leccion_completada{leccion_completada.replace('Leccion', '')}"] = True
            print(f"Lección {leccion_completada} marcada como completada en el módulo {id_modulo}.")
            ...
            with open(os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                ...
                print(
                    f"Progreso de lecciones completadas actualizado y guardado exitosamente en 'leccion_completada.json'.")
    ...
    def switch_page(self, forward=True):
        ...
        id_modulo = "modulo_2"  # Usa el número de lección para identificar el módulo
